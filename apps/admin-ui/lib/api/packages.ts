...
export const getPackages = async ({
  ...
  const url = query ? `/api/admin/packages?${query}` : "/api/admin/packages";

  return getFetcher<PackageListResponse>(url);
};

export const getPackage = async (id: string): Promise<PackageRecord> => {
  return getFetcher<PackageRecord>(`/api/admin/packages/${id}`);
};
...
export const getPackageResources = async ({
  ...
  const query = params.toString();
  const url = query
    ? `/api/admin/packages/${packageId}/resources?${query}`
    : `/api/admin/packages/${packageId}/resources`;
