...
export const getSearchParameterIndexingStatus = async ({
  ...
  const url = resourceType
    ? `/admin/search-parameters/indexing-status/${encodeURIComponent(resourceType)}`
    : "/admin/search-parameters/indexing-status";
...
> => {
  return getFetcher<SearchIndexTableStatusRecord[]>(
    "/admin/search/index-tables",
  );
};
...
export const toggleSearchParameterActive = async (
  ...
  return postFetcher<{ active: boolean }>(
    `/admin/search-parameters/${id}/toggle-active`,
  );
};
...
> => {
  return getFetcher<CompartmentMembershipRecord[]>(
    "/admin/compartments/memberships",
  );
};
